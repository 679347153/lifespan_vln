...
python log_filter.py --run "python -m RAANav.AgenticRAG.benchmark_adapter.run_temporal_habitat_loop \
  ...
  --planning-mode room \
  --nav-execution-backend auto \
  --max-goal-geodesic-per-round 2.0 \
  --max-nav-segment-length 0.5 \
  --max-snap-distance 1.0 \
  --same-floor-y-tolerance 0.75 \
...
若浏览器打开 `visual_debug/index.html` 白屏或卡死，先使用降级版：
