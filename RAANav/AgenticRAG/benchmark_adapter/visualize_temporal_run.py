...
def _pos2d_from_memory_event(record: Dict[str, Any]) -> Optional[List[float]]:
...
def _memory_timeline_payload(memory_updates: List[Dict[str, Any]]) -> Dict[str, Any]:
    events: List[Dict[str, Any]] = []
    for seq, record in enumerate(memory_updates):
        if not isinstance(record, dict):
            continue
        details = record.get("details") if isinstance(record.get("details"), dict) else {}
        pos2d = _pos2d_from_memory_event(record)
        events.append(
            {
                "seq": seq,
                ...
                "duplicate_compaction": bool(details.get("duplicate_compaction")),
            }
        )
    return {
        "events": events,
        "event_count": len(events),
        "has_explicit_round_context": any(event.get("subtask_id") and event.get("round") is not None for event in events),
    }
...
<body>
  ...
  <script>
    ...
    function renderTables(epId, subId) {{
      ...
      $('candidateTable').innerHTML = `<thead><tr><th>round</th><th>selected</th><th>score</th><th>backend</th><th>room</th><th>mode</th><th>pose</th><th>planned</th><th>final</th><th>top candidates</th></tr></thead><tbody>` +
        cands.map(r => {{ const s = r.selected_candidate || {{}}; return `<tr><td>${{r.round}}</td><td>${{esc(s.label)}}</td><td>${{fmt(s.S_final ?? s.score)}}</td><td>${{esc(s.sim_backend)}}</td><td>${{esc(r.selected_room_id||'')}}</td><td>${{esc(r.planning_mode||'')}}</td><td>${{esc(r.selected_pose_source||'')}}<br><span class="subtle">${{esc(r.room_switch_reason||'')}}</span></td><td>${{r.planned_pose ? fmt(r.planned_pose.x,2)+','+fmt(r.planned_pose.z,2) : ''}}</td><td>${{r.final_pose ? fmt(r.final_pose.x,2)+','+fmt(r.final_pose.z,2) : ''}}</td><td>${{esc((r.top_candidates||[]).slice(0,5).map(c=>`${{c.label}}:${{fmt(c.S_final ?? c.score,2)}}`).join(' | '))}}</td></tr>`; }}).join('') + '</tbody>';
    }}
...
<body>
  ...
  <main class="grid">
    ...
    <div class="grid two">
      <section>
        ...
        <div class="controls">
          <label>Memory <select id="memoryModeSelect"><option value="stage" selected>阶段累计</option><option value="delta">仅当前轮增量</option><option value="final">最终快照</option></select></label>
          <label>Round <select id="roundSelect"></select></label>
          <span class="subtle" id="memoryTimelineStatus"></span>
        </div>
  ...
  <script>
    ...
    const colors = {trajectory:'#111827', final:'#111827', target:'#dc2626', peak:'#059669', selected:'#2563eb', planned:'#2563eb', nav_requested:'#ef4444', nav_snapped:'#f59e0b', nav_safe:'#10b981', obs:'#7c3aed', memory:'#0891b2', memory_history:'#38bdf8', memory_delta:'#f97316', memory_final:'#0891b2', memory_removed:'#94a3b8', layout:'#0f766e'};
    ...
    function fillSelectors(){const epSel=$('episodeSelect');if(!data.episodes.length){epSel.innerHTML='';$('subtaskSelect').innerHTML='';$('roundSelect').innerHTML='';return;}epSel.innerHTML=data.episodes.map((e,i)=>`<option value="${esc(e.episode_id)}">${i+1}. state=${e.seen_layout_count_before} ${esc(e.layout_id)}</option>`).join('');epSel.addEventListener('change',()=>{fillSubtaskSelect();renderSelected();});$('subtaskSelect').addEventListener('change',()=>{fillRoundSelect();renderSelected();});$('roundSelect').addEventListener('change',renderSelected);$('memoryModeSelect').addEventListener('change',renderSelected);fillSubtaskSelect();}
    function fillSubtaskSelect(){const ep=$('episodeSelect').value||data.episodes[0]?.episode_id;const rows=data.groups.diagnostics_by_episode[ep]||[];$('subtaskSelect').innerHTML=rows.map((r,i)=>`<option value="${esc(r.subtask_id)}">${i+1}. ${esc(r.task_type)} / ${esc(r.target_object)} / ${r.found?'found':'miss'}</option>`).join('');fillRoundSelect();}
    ...
    function selectedMemoryMode(){return $('memoryModeSelect')?.value || 'stage';}
    function selectedRoundValue(){return $('roundSelect')?.value || '__end__';}
    function fillRoundSelect(){const subId=$('subtaskSelect').value,cands=data.groups.candidates_by_subtask[subId]||[],rounds=[...new Set(cands.map(r=>r.round).filter(r=>r!==null&&r!==undefined))].sort((a,b)=>Number(a)-Number(b));$('roundSelect').innerHTML='<option value="__end__">Subtask 结束</option>'+rounds.map(r=>`<option value="${esc(r)}">Round ${esc(r)}</option>`).join('');}
    function memoryEventPoint(event,kind){const p=event.pos_2d;if(!Array.isArray(p)||p.length<2)return null;return{kind,x:Number(p[0]),y:0.18,z:Number(p[1]),label:event.label||event.track_id||'',track_id:event.track_id,event:event.event,room_id:event.room_id,seq:event.seq,round:event.round,position_confidence:event.position_confidence,position_source:event.position_source};}
    function finalMemoryPoints(sceneName){return (sceneMemoryFor(sceneName)?.tracks||[]).map(t=>{const p=Array.isArray(t.pos_2d)?{x:t.pos_2d[0],y:0.16,z:t.pos_2d[1]}:(Array.isArray(t.pos_3d)?{x:t.pos_3d[0],y:t.pos_3d[1]||0.16,z:t.pos_3d[2]}:null);return p?{kind:'memory_final',...p,label:t.label,track_id:t.obj_id,event:'final_snapshot'}:null;}).filter(Boolean);}
    function memoryCutoff(epId,subId,roundValue){const events=data.memory_timeline?.events||[];const explicit=events.filter(e=>e.subtask_id===subId&&(roundValue==='__end__'||Number(e.round)<=Number(roundValue)));if(explicit.length){const cutoff=Math.max(...explicit.map(e=>Number(e.seq)));const deltaRound=roundValue==='__end__'?Math.max(...explicit.map(e=>Number(e.round)).filter(Number.isFinite)) : Number(roundValue);return{cutoffSeq:cutoff,deltaRound:Number.isFinite(deltaRound)?deltaRound:null,source:'explicit_context'};}const ep=data.episodes.find(e=>e.episode_id===epId)||{};const sameLayout=events.filter(e=>String(e.layout_id||'')===String(ep.layout_id||''));if(sameLayout.length){return{cutoffSeq:Math.max(...sameLayout.map(e=>Number(e.seq))),deltaRound:null,source:'layout_fallback'};}return{cutoffSeq:null,deltaRound:null,source:'final_snapshot_fallback'};}
    function memoryPointsFor(ep,subId){const mode=selectedMemoryMode();if(mode==='final')return finalMemoryPoints(ep.scene_name);const roundValue=selectedRoundValue(),events=data.memory_timeline?.events||[],cut=memoryCutoff(ep.episode_id,subId,roundValue);if(cut.cutoffSeq===null){$('memoryTimelineStatus').textContent='memory timeline unavailable; showing final snapshot.';return finalMemoryPoints(ep.scene_name);}const sameScene=e=>!ep.scene_name||!e.scene_name||String(e.scene_name)===String(ep.scene_name)||String(e.track_id||'').startsWith(String(ep.scene_name)+':');const active=events.filter(e=>sameScene(e)&&Number(e.seq)<=Number(cut.cutoffSeq));const byTrack=new Map(),removed=new Set();active.forEach(e=>{if(e.event==='duplicate_compaction'&&e.merged_track_id)removed.add(String(e.merged_track_id));if(!e.track_id||!Array.isArray(e.pos_2d))return;if(removed.has(String(e.track_id)))return;byTrack.set(String(e.track_id),e);});let deltaIds=new Set();if(cut.source==='explicit_context'){events.filter(e=>sameScene(e)&&e.subtask_id===subId&&Number(e.seq)<=Number(cut.cutoffSeq)&&(roundValue==='__end__'?Number(e.round)===Number(cut.deltaRound):Number(e.round)===Number(roundValue))).forEach(e=>{if(e.track_id)deltaIds.add(String(e.track_id));});}const pts=[];byTrack.forEach((e,id)=>{if(mode==='delta'&&!deltaIds.has(id))return;const kind=deltaIds.has(id)?'memory_delta':'memory_history';const p=memoryEventPoint(e,kind);if(p)pts.push(p);});$('memoryTimelineStatus').textContent=`memory=${mode}, source=${cut.source}, tracks=${pts.length}, events<=${cut.cutoffSeq}`;return pts;}
    function allPointsFor(epId,subId){
      ...
      candsForSub.forEach(r=>{const sp=r.selected_candidate;if(sp)pts.push({kind:'selected',x:sp.world_x,y:0.2,z:sp.world_z,label:sp.label});(r.top_candidates||[]).slice(0,5).forEach((c,i)=>pts.push({kind:'peak',x:c.world_x,y:0.1+i*0.03,z:c.world_z,label:`top${i+1} ${c.label}`}));const rp=r.requested_pose;if(rp)pts.push({kind:'nav_requested',x:rp.x,y:rp.y||0,z:rp.z,label:'requested'});const sn=r.snapped_pose;if(sn)pts.push({kind:'nav_snapped',x:sn.x,y:sn.y||0,z:sn.z,label:'snapped'});const sw=r.safe_waypoint;if(sw)pts.push({kind:'nav_safe',x:sw.x,y:sw.y||0,z:sw.z,label:'safe waypoint'});const pp=r.planned_pose;if(pp)pts.push({kind:'planned',x:pp.x,y:pp.y||0,z:pp.z,label:'planned'});const fp=r.final_pose;if(fp)pts.push({kind:'final',x:fp.x,y:fp.y||0,z:fp.z,label:'round final'});});
      ...
      memoryPointsFor(ep,subId).forEach(p=>pts.push(p));
      return pts.filter(p=>Number.isFinite(Number(p.x))&&Number.isFinite(Number(p.z)));
    }
    ...
    function render3D(points,geom){
      ...
      const isMemoryKind=p=>String(p.kind||'').startsWith('memory');
      const isCandKind=p=>['peak','selected','planned','nav_requested','nav_snapped','nav_safe'].includes(p.kind);
      const drawPts=points.filter(p=>p.kind==='obs'?layerOn('observations'):isMemoryKind(p)?layerOn('memory'):isCandKind(p)?layerOn('candidates'):['trajectory','final'].includes(p.kind)?layerOn('trajectory'):true).map(p=>({...p,y:p.kind==='obs'?L.obs:isMemoryKind(p)?L.memory:isCandKind(p)?L.cand:p.kind==='target'?L.cand+0.35:L.traj}));
      ...
      drawPts.sort((a,b)=>project3d(b,center,canvas).depth-project3d(a,center,canvas).depth).forEach(p=>{drawDot(p,colors[p.kind]||'#64748b',p.kind==='target'?7:p.kind==='obs'?2.2:isMemoryKind(p)?4.5:4.5,p.kind==='obs'?0.34:p.kind==='trajectory'?0.58:p.kind==='memory_history'?0.62:0.9);if(p.kind==='target'||p.kind==='selected')drawLabel(p.label,p,colors[p.kind]||'#475569');});
      if(traj.length){drawLabel('start',traj[0],'#92400e');drawLabel('end',traj[traj.length-1],'#111827');}
      const memoryPts=drawPts.filter(isMemoryKind).slice(0,160);
