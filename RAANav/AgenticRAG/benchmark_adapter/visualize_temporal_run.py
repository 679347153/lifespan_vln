...
<body>
  ...
  <script>
    ...
    const colors = {trajectory:'#111827', final:'#111827', target:'#dc2626', peak:'#059669', selected:'#2563eb', planned:'#2563eb', obs:'#7c3aed', memory:'#0891b2', memory_history:'#38bdf8', memory_delta:'#f97316', memory_final:'#0891b2', memory_removed:'#94a3b8', layout:'#0f766e'};
    ...
    function render3D(points,geom){
      ...
      const drawPts=points.filter(p=>p.kind==='obs'?layerOn('observations'):isMemoryKind(p)?layerOn('memory'):['peak','selected','planned'].includes(p.kind)?layerOn('candidates'):['trajectory','final'].includes(p.kind)?layerOn('trajectory'):true).map(p=>({...p,y:p.kind==='obs'?L.obs:isMemoryKind(p)?L.memory:['peak','selected','planned'].includes(p.kind)?L.cand:p.kind==='target'?L.cand+0.35:L.traj}));
